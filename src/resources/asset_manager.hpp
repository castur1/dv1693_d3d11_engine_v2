...
class AssetHandle {
    ...
    // TODO: This pointer only valid as long as the asset don't get unloaded.
...
public:
    ...
    AssetID GetID() const { return this->uuid; }

    void SetID(const AssetID &uuid) {
        if (this->uuid == uuid)
            return;

        this->uuid = uuid;
        this->cachedPtr = nullptr;
    }
};
